...
@interface LMVideoPlayer ()<LMAVPlayerManagerDelegate, LMPlayerControlViewDelagate, LMPortraitControlViewDelegate, LMLandScapeControlViewDelegate, LMVideoPlayerViewDelagate, LMLoadingViewDelegate, LMCoverControlViewDelegate>
...
// AVPlayer 管理
@property (nonatomic, strong) LMAVPlayerManager *playerMgr;
// 播放数据模型
...
+ (instancetype)videoPlayerWithView:(UIView *)view delegate:(id<LMVideoPlayerDelegate>)delegate playerModel:(LMPlayerModel *)playerModel {
    ...
    // !!!: 创建AVPlayer管理
    instance.playerMgr = [LMAVPlayerManager playerManagerWithDelegate:instance playerStatusModel:instance.playerStatusModel];
...
- (void)portraitProgressSliderEndDrag:(CGFloat)value {
    ...
    [self.playerMgr seekToTime:dragedSeconds completionHandler:^(BOOL finished) {
